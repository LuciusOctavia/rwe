    ...
    int CobExecutionContext::getValueInternal(CobValueId valueId, int arg1, int arg2, int arg3, int arg4)
    {
        switch (valueId)
        {
            ...
            case CobValueId::XZAtan:
            {
                ...
                auto result = std::atan2(pair.second, pair.first);
            ...
            case CobValueId::XZHypot:
            {
                auto coords = arg1;
                auto pair = unpackCoords(coords);
                auto result = std::sqrt((pair.first * pair.first) + (pair.second * pair.second));
                return toFixedPoint(result);
            }
            case CobValueId::Atan:
            {
                auto a = arg1;
                auto b = arg2;
                return toFixedPoint(std::atan2(fromFixedPoint(a), fromFixedPoint(b)));
            }
